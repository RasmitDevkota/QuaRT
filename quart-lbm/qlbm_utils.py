...
def compute_memory_requirements(
    ...
    # AS
    if include_AS:
        ancilla_idxs_AS = list(range(n_qubits_ancilla, n_qubits_ancilla+1))
        n_qubits_ancilla += 1
    else:
        ancilla_idxs_AS = None

    # AE
    ancilla_idxs_AE = list(range(n_qubits_ancilla, n_qubits_ancilla+2))
    n_qubits_ancilla += 2
    ...
    # BC
    if include_BC:
        ancilla_idxs_BC = list(range(n_qubits_ancilla, n_qubits_ancilla+1))
        n_qubits_ancilla += 1
    else:
